...
{
    public class GetProgressDisciplineDetailFunction
    {
        private readonly ILogger<GetProgressDisciplineDetailFunction> _logger;
        private readonly IProgressReportService _service;
        public GetProgressDisciplineDetailFunction(ILogger<GetProgressDisciplineDetailFunction> logger, IProgressReportService service)
        { _logger = logger; _service = service; }

        [Function("Progress_GetDisciplineDetailFunction")]
        public async Task<HttpResponseData> RunAsync(
            [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "progress/discipline")] HttpRequestData req,
            FunctionContext ctx)
        {
            var logger = ctx.GetLogger("Progress_GetDisciplineDetailFunction");
            ...
            try
            {
                ...
                {
                    logger.LogWarning(readEx, "Fallo al leer body discipline, usando {}" );
                    body = "{}";
                }
                ...
                {
                    logger.LogWarning(parseEx, "JSON inválido en discipline, request vacío");
                ...
                var result = await _service.GetDisciplineDetailAsync(request);
                var ok = req.CreateResponse(System.Net.HttpStatusCode.OK);
                await ok.WriteAsJsonAsync(new ApiResponse<DisciplineDetailResponse>{ Success=result.Success, Message=result.Message, Data=result.Data, StatusCode=result.Success?StatusCodes.Status200OK:StatusCodes.Status400BadRequest});
                return ok;
            }
            catch (System.Exception ex)
            {
                logger.LogError(ex, "Error detalle disciplina");
