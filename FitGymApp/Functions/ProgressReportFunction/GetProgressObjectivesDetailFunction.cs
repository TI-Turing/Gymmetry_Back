...
{
    public class GetProgressObjectivesDetailFunction
    {
        private readonly ILogger<GetProgressObjectivesDetailFunction> _logger;
        private readonly IProgressReportService _service;
        public GetProgressObjectivesDetailFunction(ILogger<GetProgressObjectivesDetailFunction> logger, IProgressReportService service)
        {
            _logger = logger;
            _service = service;
        }
            ...
            FunctionContext ctx)
        {
            ...
            try
            {
                ...
                {
                    logger.LogWarning(readEx, "Fallo al leer body objectives, usando {}" );
                    body = "{}";
                }
                ...
                {
                    logger.LogWarning(parseEx, "JSON inválido en objectives, request vacío");
                ...
                var result = await _service.GetObjectivesDetailAsync(request);
                var ok = req.CreateResponse(System.Net.HttpStatusCode.OK);
                await ok.WriteAsJsonAsync(new ApiResponse<ObjectivesDetailResponse>{ Success = result.Success, Message = result.Message, Data = result.Data, StatusCode = result.Success ? StatusCodes.Status200OK : StatusCodes.Status400BadRequest });
                return ok;
            }
            catch (System.Exception ex)
            {
                logger.LogError(ex, "Error detalle objetivos");
