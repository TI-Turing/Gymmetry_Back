...
public class AddRoutineAssignedFunction
{
    ...
    [Function("AddRoutineAssignedFunction")]
    public async Task<ApiResponse<Guid>> AddAsync([HttpTrigger(AuthorizationLevel.Function, "post", Route = "routineassigned/add")] HttpRequest req)
    {
        ...
        _logger.LogInformation("Procesando solicitud para agregar un RoutineAssigned.");
        try
        {
            ...
            var result = _service.CreateRoutineAssigned(objRequest);
            if (!result.Success)
            {
                return new ApiResponse<Guid>
                {
        ...
        {
            _logger.LogError(ex, "Error al agregar RoutineAssigned.");
            return new ApiResponse<Guid>
            {
                Success = false,
