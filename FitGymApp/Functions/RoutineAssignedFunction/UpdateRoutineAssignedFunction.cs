...
public class UpdateRoutineAssignedFunction
{
    private readonly ILogger<UpdateRoutineAssignedFunction> _logger;
    private readonly IRoutineAssignedService _service;

    public UpdateRoutineAssignedFunction(ILogger<UpdateRoutineAssignedFunction> logger, IRoutineAssignedService service)
    {
        _logger = logger;
        _service = service;
    }
        ...
        FunctionContext executionContext)
    {
        ...
        logger.LogInformation("Procesando solicitud para actualizar un RoutineAssigned.");
        ...
        try
        {
            ...
            var objRequest = JsonConvert.DeserializeObject<UpdateRoutineAssignedRequest>(requestBody);
            var validationResult = ModelValidator.ValidateModel<UpdateRoutineAssignedRequest, Guid>(objRequest, StatusCodes.Status400BadRequest);
            ...
            var result = await _service.UpdateRoutineAssignedAsync(objRequest, userId, ip, invocationId);
            if (!result.Success)
            {
        ...
        {
            logger.LogError(ex, "Error al actualizar RoutineAssigned.");
