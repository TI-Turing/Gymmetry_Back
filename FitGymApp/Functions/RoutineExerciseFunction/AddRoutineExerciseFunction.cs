...
public class AddRoutineExerciseFunction
{
    ...
    [Function("AddRoutineExerciseFunction")]
    public async Task<ApiResponse<Guid>> AddAsync([HttpTrigger(AuthorizationLevel.Function, "post", Route = "routineexercise/add")] HttpRequest req)
    {
        ...
        _logger.LogInformation("Procesando solicitud para agregar un RoutineExercise.");
        try
        {
            ...
            var result = _service.CreateRoutineExercise(objRequest);
            if (!result.Success)
            {
                return new ApiResponse<Guid>
                {
        ...
        {
            _logger.LogError(ex, "Error al agregar RoutineExercise.");
            return new ApiResponse<Guid>
            {
                Success = false,
