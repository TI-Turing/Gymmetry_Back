...
public class AddRoutineTemplateFunction
{
    ...
    [Function("AddRoutineTemplateFunction")]
    public async Task<ApiResponse<Guid>> AddAsync([HttpTrigger(AuthorizationLevel.Function, "post", Route = "routinetemplate/add")] HttpRequest req)
    {
        ...
        _logger.LogInformation("Procesando solicitud para agregar un RoutineTemplate.");
        try
        {
            ...
            var result = _service.CreateRoutineTemplate(objRequest);
            if (!result.Success)
            {
                return new ApiResponse<Guid>
                {
        ...
        {
            _logger.LogError(ex, "Error al agregar RoutineTemplate.");
            return new ApiResponse<Guid>
            {
                Success = false,
