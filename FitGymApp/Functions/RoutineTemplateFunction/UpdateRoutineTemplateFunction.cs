...
public class UpdateRoutineTemplateFunction
{
    ...
    [Function("UpdateRoutineTemplateFunction")]
    public async Task<ApiResponse<Guid>> UpdateAsync([HttpTrigger(AuthorizationLevel.Function, "put", Route = "routinetemplate/update")] HttpRequest req)
    {
        ...
        _logger.LogInformation("Procesando solicitud para actualizar un RoutineTemplate.");
        try
        {
            ...
            var result = _service.UpdateRoutineTemplate(objRequest);
            if (!result.Success)
            {
                return new ApiResponse<Guid>
                {
        ...
        {
            _logger.LogError(ex, "Error al actualizar RoutineTemplate.");
            return new ApiResponse<Guid>
            {
                Success = false,
