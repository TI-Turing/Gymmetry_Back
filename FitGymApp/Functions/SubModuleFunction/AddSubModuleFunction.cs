...
public class AddSubModuleFunction
{
    ...
    [Function("AddSubModuleFunction")]
    public async Task<ApiResponse<Guid>> AddAsync([HttpTrigger(AuthorizationLevel.Function, "post", Route = "submodule/add")] HttpRequest req)
    {
        ...
        _logger.LogInformation("Procesando solicitud para agregar un SubModule.");
        try
        {
            ...
            var result = _service.CreateSubModule(objRequest);
            if (!result.Success)
            {
                return new ApiResponse<Guid>
                {
        ...
        {
            _logger.LogError(ex, "Error al agregar SubModule.");
            return new ApiResponse<Guid>
            {
                Success = false,
