...
public class AddPlanFunction
{
    ...
    public async Task<ApiResponse<Guid>> AddAsync([HttpTrigger(AuthorizationLevel.Function, "post", Route = "plan/add")] HttpRequest req)
    {
        ...
        _logger.LogInformation("Procesando solicitud para agregar un Plan.");
        try
        {
            ...
            var result = _service.CreatePlan(objRequest);
            if (!result.Success)
            {
                return new ApiResponse<Guid>
                {
        ...
        {
            _logger.LogError(ex, "Error al agregar Plan.");
            return new ApiResponse<Guid>
            {
                Success = false,
