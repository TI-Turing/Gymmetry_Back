...
namespace FitGymApp.Functions.PlanTypeFunction
{
    public class DeletePlanTypeFunction
    {
        ...
        public async Task<HttpResponseData> RunAsync(
            ...
            Guid id)
        {
            var logger = executionContext.GetLogger("PlanType_DeletePlanTypeFunction");
            logger.LogInformation($"Procesando solicitud de borrado para PlanType {id}");
            ...
            try
            {
                var result = await _service.DeletePlanTypeAsync(id);
                if (!result.Success)
                {
            ...
            {
                logger.LogError(ex, "Error al eliminar PlanType.");
