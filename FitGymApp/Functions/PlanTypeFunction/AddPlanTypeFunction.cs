...
public class AddPlanTypeFunction
{
    ...
    [Function("AddPlanTypeFunction")]
    public async Task<ApiResponse<Guid>> AddAsync([HttpTrigger(AuthorizationLevel.Function, "post", Route = "plantype/add")] HttpRequest req)
    {
        ...
        _logger.LogInformation("Procesando solicitud para agregar un PlanType.");
        try
        {
            ...
            var result = _service.CreatePlanType(objRequest);
            if (!result.Success)
            {
                return new ApiResponse<Guid>
                {
        ...
        {
            _logger.LogError(ex, "Error al agregar PlanType.");
            return new ApiResponse<Guid>
            {
                Success = false,
