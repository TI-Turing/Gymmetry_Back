...
public class UpdateGymTypeFunction
{
    ...
    [Function("UpdateGymTypeFunction")]
    public async Task<ApiResponse<Guid>> UpdateAsync([HttpTrigger(AuthorizationLevel.Function, "put", Route = "gymtype/update")] HttpRequest req)
    {
        ...
        _logger.LogInformation("Procesando solicitud para actualizar un GymType.");
        try
        {
            ...
            var result = _service.UpdateGymType(objRequest);
            if (!result.Success)
            {
                return new ApiResponse<Guid>
                {
        ...
        {
            _logger.LogError(ex, "Error al actualizar GymType.");
            return new ApiResponse<Guid>
            {
                Success = false,
