...
public class AddUserTypeFunction
{
    ...
    [Function("AddUserTypeFunction")]
    public async Task<ApiResponse<Guid>> AddAsync([HttpTrigger(AuthorizationLevel.Function, "post", Route = "usertype/add")] HttpRequest req)
    {
        ...
        _logger.LogInformation("Procesando solicitud para agregar un UserType.");
        try
        {
            ...
            var result = _service.CreateUserType(objRequest);
            if (!result.Success)
            {
                return new ApiResponse<Guid>
                {
        ...
        {
            _logger.LogError(ex, "Error al agregar UserType.");
            return new ApiResponse<Guid>
            {
                Success = false,
