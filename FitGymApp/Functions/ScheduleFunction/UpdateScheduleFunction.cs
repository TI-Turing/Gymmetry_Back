...
public class UpdateScheduleFunction
{
    ...
    public async Task<ApiResponse<Guid>> UpdateAsync([HttpTrigger(AuthorizationLevel.Function, "put", Route = "schedule/update")] HttpRequest req)
    {
        ...
        _logger.LogInformation("Procesando solicitud para actualizar un Schedule.");
        try
        {
            ...
            var result = _service.UpdateSchedule(objRequest);
            if (!result.Success)
            {
                return new ApiResponse<Guid>
                {
        ...
        {
            _logger.LogError(ex, "Error al actualizar Schedule.");
            return new ApiResponse<Guid>
            {
                Success = false,
