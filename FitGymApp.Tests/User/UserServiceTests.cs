...
namespace FitGymApp.Tests.User
{
    public class UserServiceTests
    {
        ...
        public async Task CreateUserAsync_ReturnsError_WhenEmailExists()
        {
            ...
            _userRepositoryMock.Setup(r => r.FindUsersByFieldsAsync(It.IsAny<Dictionary<string, object>>()))
                .ReturnsAsync((IEnumerable<FitGymApp.Domain.Models.User>)new List<FitGymApp.Domain.Models.User> { new FitGymApp.Domain.Models.User { Email = request.Email } });
        ...
        public async Task CreateUserAsync_ReturnsError_WhenPasswordHashFails()
        {
            ...
            _userRepositoryMock.Setup(r => r.FindUsersByFieldsAsync(It.IsAny<Dictionary<string, object>>()))
                .ReturnsAsync((IEnumerable<FitGymApp.Domain.Models.User>)new List<FitGymApp.Domain.Models.User>());
        ...
        public async Task GetUserByIdAsync_ReturnsUser_WhenFound()
        {
            ...
            var user = new FitGymApp.Domain.Models.User { Id = userId, Email = "a@b.com", Password = "hash" };
        ...
        {
            var userId = Guid.NewGuid();
            _userRepositoryMock.Setup(r => r.GetUserByIdAsync(userId)).ReturnsAsync((FitGymApp.Domain.Models.User?)null);
            var result = await _service.GetUserByIdAsync(userId);
            Assert.False(result.Success);
        ...
        {
            var users = new List<FitGymApp.Domain.Models.User> { new FitGymApp.Domain.Models.User { Email = "a@b.com", Password = "hash" } };
        ...
        public async Task UpdateUserAsync_ReturnsNotFound_WhenUserDoesNotExist()
        {
            ...
            _userRepositoryMock.Setup(r => r.GetUserByIdAsync(req.Id)).ReturnsAsync((FitGymApp.Domain.Models.User?)null);
        ...
        {
            var userId = Guid.NewGuid();
            var before = new FitGymApp.Domain.Models.User { Id = userId, Email = "a@b.com", Password = "hash" };
            var req = new UpdateRequest { Id = userId };
            _userRepositoryMock.Setup(r => r.GetUserByIdAsync(userId)).ReturnsAsync(before);
            _userRepositoryMock.Setup(r => r.UpdateUserAsync(It.IsAny<FitGymApp.Domain.Models.User>())).ReturnsAsync(true);
        ...
        public async Task UpdateUserAsync_ReturnsError_WhenUpdateFails()
        {
            ...
            var req = new UpdateRequest { Id = userId };
            _userRepositoryMock.Setup(r => r.GetUserByIdAsync(userId)).ReturnsAsync(before);
            _userRepositoryMock.Setup(r => r.UpdateUserAsync(It.IsAny<FitGymApp.Domain.Models.User>())).ReturnsAsync(false);
            var result = await _service.UpdateUserAsync(req);
            Assert.False(result.Success);
        }
        ...
        public async Task UpdatePasswordAsync_ReturnsError_WhenUserNotFound()
        {
            ...
            _userRepositoryMock.Setup(r => r.FindUsersByFieldsAsync(It.IsAny<Dictionary<string, object>>())).ReturnsAsync(new List<FitGymApp.Domain.Models.User>());
        ...
        {
            var req = new PasswordUserRequest { Email = "a@b.com", NewPassword = "newpass", Token = "t" };
            var user = new FitGymApp.Domain.Models.User { Email = req.Email, Password = "oldhash" };
            _userRepositoryMock.Setup(r => r.FindUsersByFieldsAsync(It.IsAny<Dictionary<string, object>>())).ReturnsAsync(new List<FitGymApp.Domain.Models.User> { user });
        ...
        {
            var gymId = Guid.NewGuid();
            var users = new List<FitGymApp.Domain.Models.User> { new FitGymApp.Domain.Models.User { Id = Guid.NewGuid() } };
        ...
        {
            var req = new FitGymApp.Domain.DTO.User.Request.UploadUserProfileImageRequest { UserId = Guid.NewGuid(), Image = new byte[] { 1, 2, 3 } };
